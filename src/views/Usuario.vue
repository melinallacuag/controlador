        ...
        <div class="mb-4">
            ...
            <div class="flex space-x-2">
                <select v-model="fRol" @change="filtrarUsuarios"
                    ...
                    <option v-for="rol in roles" :key="rol.id_rol" :value="rol.nombre" class="text-sm">
                     {{ rol.nombre }} 
                    </option>
                </select>
            ...
            <table class="min-w-full border border-gray-[#212529] dark:border-gray-[#212529] rounded-lg">
                ...
                <tbody>
                    <template v-for="usuario in usuariosFiltrados" :key="usuario.id_usuario">
                        <tr class="dark:border-gray-300 uppercase text-xs font-medium">
                            <td class="px-2 py-2 border border-gray-300">
                                <div class="flex items-center">
                                    <button @click="toggleDetalles(usuario.id_usuario)"
                            ...
                            <td class="px-2 py-2 border border-gray-300 hidden sm:table-cell">
                               <div :class="{'bg-[#008000] text-white text-center': usuario.estado == '1',
                                'text-white text-center bg-[#fb2c36]': usuario.estado == '0'}" class="p-2 rounded text-xs">
                        ...
                        <tr v-if="detallesVisible === usuario.id_usuario"
                            ...
                            <td colspan="4" class="border border-gray-300 px-4 py-2">
                                ...
                                <div>Estado: {{  usuario.estado == '1' ? 'ACTIVO' : 'INACTIVO' }} </div>
                            </td>
                        </tr>
                    </template>
                </tbody>
...
export default {
    ...
    computed: {
        usuariosFiltrados() {
            ...
            const filtered = this.usuarios.filter(usuario => {
                ...
                const matchesUser = usuario.usuario.toLowerCase().includes(this.fUsuario.toLowerCase());

                const matchesBusqueda = matchesNombre || matchesUser;
    ...
    methods: {
        ...
        async obtenerUsuarios() {
            ...
            try {
                const response = await api.get(`/usuario/listar.php`);
        ...
        editarUsuario(usuario) {
            this.$router.push({ name: 'EditarUsuario', query: { usuarioId: usuario.id_usuario } });
        },
        crearUsuario() {
